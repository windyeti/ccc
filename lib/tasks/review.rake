namespace :review do
  task create: :environment do
    ...
    rows.each do |row|
      ...
      unless already_id.include?(tov.id)
        tov.reviews.each {|review| api_create_review(review, id)}
        already_id << tov.id
      end
    end
  ...
  def api_create_review(review, id)
    ...
    # p review.tov.title
    # p data = {"review": {"product_id": 992, "author": "Инга", "email": "author@mail.com"}}
    p data = {
      review: {
        product_id: id,
        author: review[:author],
        email: "author@mail.com",
        content: review[:date_published] + " " + review[:text],
        published: true,
        rating: review[:rating].to_i * 2
      }
    }

    url_api_category = "http://#{api_key}:#{password}@#{domain}/admin/reviews.json"
