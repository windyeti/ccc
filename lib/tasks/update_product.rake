namespace :parsing do
  task update_product: :environment do
    ...
    # response = api_get_products(page)
    # body = JSON.parse(response.body)
    # File.write("#{Rails.public_path}/100_products.txt", body)
    # File.write("#{Rails.public_path}/1_products.txt", body.first)
    #
    #
    # products_no_image = products.map {|p| {title: p['title'], link: p['permalink']} if p['images'].size == 0}.compact
    # File.write("#{Rails.public_path}/no_image.txt", products_no_image.join("\n"))
    # insales = products.map {|p| {title: p['title'], link: p['permalink']} }
    # my = Tov.all.map {|p| {title: p.title, link: p.link}}
    # compare_arrays(insales, my)
    #
    # my = Tov.all
    # product = products.find { |p| p if p['title'] == 'Ботинки DC EVAN HI WNT M SHOE WE9 WHEAT' }
    # # product = products.find { |p| p if p['title'] == 'Детские Ботинки Сноубордические PRIME Fun Child' }
    # find_for_update([product], my)

    my = Tov.all
    find_for_update(products, my)
  end
  ...
  def api_get_products(page)
    ...
    p url_api_category = "http://#{api_key}:#{password}@#{domain}/admin/products.json?per_page=100&page=#{page}"

    RestClient.get( url_api_category )
  end

  def api_update_product(item_insales)
    api_key = Rails.application.credentials[:shop][:api_key]
    password = Rails.application.credentials[:shop][:password]
    domain = Rails.application.credentials[:shop][:domain]
    ...
    data = {
      "product": {
        "images": item_insales['images'],
        "description": item_insales['description']
      }
    }

    p url_api_category = "http://#{api_key}:#{password}@#{domain}/admin/products/#{id}.json"
